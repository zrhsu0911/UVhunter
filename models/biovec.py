...
def generate_corpusfile(corpus_fname, n, out):
    ...
    for r in SeqIO.parse(corpus_fname, "fasta"):
        ngram_patterns = split_ngrams((r.seq).upper(), n)
...
class ProtVec(word2vec.Word2Vec):
    ...
    def __init__(self, corpus_fname=None, corpus=None, n=3, size=100, out="corpus.txt",  sg=1, window=25, min_count=2, workers=3, epochs=20):
        """
        ...
        word2vec.Word2Vec.__init__(self, corpus, size=size, sg=sg, window=window, min_count=min_count, workers=workers, iter=epochs)
        ...
        for ngrams in ngram_patterns:
            ...
            for ngram in ngrams:
                try:
                    ...
                    print("Model has never trained this n-gram: " + ngram)
                    continue
